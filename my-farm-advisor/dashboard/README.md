# Row Crop Intelligence Dashboard — Assignment 3

A Plotly Dash application that aligns Sentinel-2 NDVI, NASA POWER daily weather, and
cumulative GDD for **Field osm-1219926116 (Field 1) — 2023 Corn** across a shared
seasonal timeline. The CDL (Cropland Data Layer) confirms this field was planted to
**Corn (95% composition)** in 2023.
...
1. **Last spring frost** — Apr 26 (Tmin −1.3°C), visible in Temperature panel
...
The dashboard integrates Sentinel-2 NDVI, NASA POWER daily weather, and USDA CDL crop
classification into a **four-panel aligned timeline** for the selected field-year. A reader
can scan vertically across the same dates to compare vegetation changes, rainfall,
temperature events, and accumulated growing degree days.

### Dashboard Panels

1. **NDVI** — 9 Sentinel-2 scenes across the 2023 growing season, with peak and green-up annotations
2. **Daily Precipitation (mm)** — NASA POWER daily rainfall, heaviest events annotated
3. **Temperature (°C)** — Daily Tmax/Tmin with filled range, frost and heat wave annotations
4. **Cumulative GDD (°C·day)** — Base 10°C, cap 30°C, accumulated from May 1 planting

### Additional Sections

1. **KPI Summary** - Field count, total acreage, average NDVI, average rainfall, soil health score, sustainability index
2. **Exploratory Visualizations** — Soil pH distribution, NDVI comparison across fields, correlation matrix
3. **Geospatial Map** — Field boundaries colored by soil health score with interactive hover tooltips
4. **Weather/Climate Analysis** — Monthly precipitation trends and growing season highlights
...
output is the **aligned timeline** — a four-panel figure with a shared date axis
showing NDVI, precipitation, temperature, and cumulative GDD for the selected
field-year. Optionally, it can export a static HTML file with `--export`.
...
dashboard/
  ...
  dashboard_data.json       - Pre-computed data package (6.3 MB)
  src/
    row_crop_dashboard.py   - Main Dash application (aligned timeline + all panels)
...
The aligned timeline tracks Field 1 through the 2023 growing season. NDVI rose
